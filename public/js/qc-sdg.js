(function () {
	...
	customElements.define('qc-outer', create_custom_element(
		OuterWC,
		{
	...
	var root_1 = from_html(`<qc-inner></qc-inner>`, 2);
	var on_click = (_, inners) => get(inners).push('foo' + get(inners).length);
	var root = from_html(`<div> </div> <qc-outer></qc-outer> <input/> <button>Ajouter un inner</button>`, 3);

	function WrapperWC($$anchor, $$props) {
		push($$props, true);

		let inners = state(proxy(['foo1', "foo2"]));
		let shared = state('shared');
		let outer = state(proxy({}));

		user_effect((_) => {
			console.log("wrapper update", get(outer), get(shared));
			get(outer).setAttribute('shared', get(shared));
		});
		...
		var qc_outer = sibling(div, 2);

		set_custom_element_data(qc_outer, 'foo', 'foo');

		each(qc_outer, 20, () => get(inners), (foo) => foo, ($$anchor, foo) => {
			var qc_inner = root_1();

			template_effect(() => set_custom_element_data(qc_inner, 'foo', foo));
			append($$anchor, qc_inner);
		});

		reset(qc_outer);
		bind_this(qc_outer, ($$value) => set(outer, $$value), () => get(outer));

		var input = sibling(qc_outer, 2);

		remove_input_defaults(input);

		var button = sibling(input, 2);

		button.__click = [on_click, inners];
		template_effect(() => set_text(text, `shared: ${get(shared) ?? ''}`));
		bind_value(input, () => get(shared), ($$value) => set(shared, $$value));
		append($$anchor, fragment);
		pop();
	}

	delegate(['click']);
	customElements.define('qc-wrapper', create_custom_element(WrapperWC, {}, [], [], false));
