(function () {
	...
	var root_1$2 = add_locations(template(`<div class="go-to-content"><a> </a></div>`), PivHeader[FILENAME], [[65, 6, [[66, 8]]]]);
	...
	var root_8$1 = add_locations(template(`<li><a> </a></li>`), PivHeader[FILENAME], [[121, 32, [[121, 36]]]]);
	var root_6$1 = add_locations(template(`<nav><ul><!> <!></ul></nav>`), PivHeader[FILENAME], [[115, 20, [[116, 24]]]]);
	var root_9$1 = add_locations(template(`<div class="search-zone"><!></div>`), PivHeader[FILENAME], [[133, 10]]);
	...
	var root_1$1 = add_locations(template(`<div role="alert"><div><div class="qc-general-alert-elements"><!> <div class="qc-alert-content"><!> <!></div> <!></div></div></div>`), Alert[FILENAME], [
		[
			40,
			4,
			[
	...
	var root$4 = add_locations(template(`<!> <link rel="stylesheet">`, 1), Alert[FILENAME], [[68, 0]]);
	...
	var root$3 = add_locations(template(`<a href="#top"><!> <span> </span></a>`), ToTop[FILENAME], [[69, 0, [[79, 3]]]]);
	...
	var root$2 = add_locations(template(`<span role="img" class="qc-ext-link-img"></span>`), ExternalLink[FILENAME], [[89, 0]]);
	...
	var root$1 = add_locations(template(`<div class="qc-search-input"><input> <!></div>`), SearchInput[FILENAME], [[18, 0, [[19, 4]]]]);
	...
	var root = add_locations(template(`<div><!> <!></div>`), SearchBar[FILENAME], [[58, 0]]);
	...
	SelectBox[FILENAME] = 'src/sdg/components/SelectBox/SelectBox.svelte';

	var root_2 = add_locations(template(`<span class="required mandatory" aria-hidden="true">*</span>`), SelectBox[FILENAME], [[423, 26]]);
	var root_3 = add_locations(template(`<input type="search" tabindex="0">`), SelectBox[FILENAME], [[450, 20]]);

	var root_5 = add_locations(template(`<li class="form-check" tabindex="0" role="option"><label><input class="form-check-input" type="checkbox" tabindex="-1"> </label></li>`), SelectBox[FILENAME], [
		[
			468,
			28,
			[[476, 32, [[477, 36]]]]
	...
	var root_7 = add_locations(template(`<span class="sr-only"> </span>`), SelectBox[FILENAME], [[496, 36]]);
	var root_6 = add_locations(template(`<li tabindex="0" role="option"><!> </li>`), SelectBox[FILENAME], [[488, 28]]);
	var root_8 = add_locations(template(`<span role="alert" class="combo-error"><span class="sr-only">erreur</span> </span>`), SelectBox[FILENAME], [[507, 4, [[508, 30]]]]);
	var root_9 = add_locations(template(`<link rel="stylesheet">`), SelectBox[FILENAME], [[513, 0]]);

	var root_1 = add_locations(template(`<input type="hidden"> <div><label> <!></label> <div class="filter-box"><div><span> </span></div> <div class="panel-wrapper"><div class="panel"><!> <ul role="listbox" tabindex="-1"></ul></div></div></div> <!></div> <!>`, 1), SelectBox[FILENAME], [
		[392, 0],
		[
			399,
			0,
	...
	function SelectBox($$anchor, $$props) {
		...
		const selectedElementId = mutable_source();
		const binding_group = [];
		const dispatch = createEventDispatcher();

		let id = prop($$props, 'id', 28, () => Math.floor(Math.random() * 1000)),
			label = prop($$props, 'label', 12, ''),
			value = prop($$props, 'value', 12),
			items = prop($$props, 'items', 12),
			noValueMessage = prop($$props, 'noValueMessage', 12, ''),
			noOptionsMessage = prop($$props, 'noOptionsMessage', 12, ' Aucune option disponible'),
			enableSearch = prop($$props, 'enableSearch', 12, true),
			comboAriaLabel = prop($$props, 'comboAriaLabel', 12, ''),
			ariaRequired = prop($$props, 'ariaRequired', 12, false),
			error = prop($$props, 'error', 12, ''),
			searchPlaceholder = prop($$props, 'searchPlaceholder', 12, ''),
			emptyOptionSrMessage = prop($$props, 'emptyOptionSrMessage', 12, ''),
			isMultiSelect = prop($$props, 'isMultiSelect', 12, false),
			cssPath = prop($$props, 'cssPath', 12, "/assets/pgu_web_components/Css/ComboSelectBox.css");
		...
		let hidden = mutable_source(true),
			...
			search = mutable_source(''),
			filteredOptions = mutable_source([]),
			focusIn = mutable_source(false),
			comboButton = mutable_source(),
			searchInput = mutable_source(),
			listBox = mutable_source(),
			ariaActiveDescendant = mutable_source(),
			labelAsValue = mutable_source(),
			options = mutable_source(),
			cleanOptions = mutable_source(),
			valueLabel = mutable_source(''),
			root = mutable_source(),
			lastValue,
			rootInput = mutable_source();

		onMount((_) => {
			get(rootInput).parentNode instanceof ShadowRoot;
		});
		...
		function select(option) {
			if (isMultiSelect()) {
				if (!value().includes(getOptionValue(option))) {
					...
					value(value().filter((item) => strict_equals(item, getOptionValue(option), false)));
				}
			} else {
			...
			set(valueLabel, getOptionLabel(option));
			set(search, '');
			...
			if (!get(root)) {
				// peut arriver en cas de destruction/reconstruction du composant
				return;
			}

			if (!isMultiSelect()) {
				get(comboButton).focus();
			}

			let e = new CustomEvent('select', { composed: true });

			e.value = value();
			get(root).dispatchEvent(e);
		}
			...
			switch (e.code) {
				...
				case 'ArrowDown':
					...
					setTimeout(
						() => {
							if (value()) {
								get(listBox).querySelector('[aria-selected=true]')?.focus();
							} else if (enableSearch()) {
								get(searchInput).focus();
							} else {
								get(listBox).firstChild?.focus();
							}
						},
						10
					);
		...
		function handleTextKeyDown(e) {
			...
			setTimeout(() => {
				if (enableSearch()) {
					...
					set(search, get(search) + e.key);
					focusOnMatchingOption();
				}
			});
		...
		function checkOutterEvent(e) {
			if (equals$1(e.innerEventFromFilter ?? -1, id(), false)) {
				set(hidden, true);
			}
		}
		...
		function getOptionElementId(option) {
			return 'id-' + get(options).indexOf(option);
		}
		...
		function getOptionValue(option) {
			return get(labelAsValue) ? getOptionLabel(option) : option[0];
		}

		function focusOnMatchingOption(e) {
			let filteredOptions = searchIntoOptions(get(options), get(search));

			// console.log(search, filteredOptions)
			if (filteredOptions.length) {
				let optionElementId = getOptionElementId(filteredOptions[0]);

				get(listBox).querySelector('#' + optionElementId).focus(); // console.log('focusing on option')
			}
		}

		function searchIntoOptions(options, search) {
			return options.filter((option) => get(cleanOptions)[getOptionValue(option)].includes(Utils.cleanWord(search)));
		}

		function selectOption(value) {
			// console.log('selecting option value', value, typeof value);
			if (strict_equals(value, null)) {
				return;
			}

			get(options)?.forEach((o) => {
				if (equals$1(getOptionValue(o), value)) {
					select(o);
				}
			});
		}
		...
		function getOptions(items) {
			...
			if (Array.isArray(items)) {
				return items.length && Array.isArray(items[0]) ? items : Object.entries(items); // array of array, we assume items are in entries format
				// just array, so we take the entries
			}

			if (strict_equals(typeof items, 'object')) {
				let options = [];
		...
		function refreshValue(options) {
			if (Array.isArray(lastValue)) {
				value(options.filter((option) => lastValue.includes(getOptionValue(option))));
				value(value().length > 0 ? lastValue : []);
			} else {
				value(options.find((option) => strict_equals(getOptionValue(option), lastValue)) ? lastValue : null);
			}
		}
		...
		function topText(
			...
			if (equals$1(options.length, 0)) {
				return noOptionsMessage;
			}

			// ne pas afficher la valeur sélectionnée quand on a une sélection multiple
			if (value && strict_equals(valueLabel, "", false) && !isMultiSelect()) {
				return valueLabel;
			}
		...
		legacy_pre_effect(() => (deep_read_state(items())), () => {
			set(labelAsValue, Array.isArray(items()) && items().length > 0 && !Array.isArray(items()[0]));
		});

		legacy_pre_effect(() => (deep_read_state(items())), () => {
			set(options, getOptions(items()));
		});

		legacy_pre_effect(() => (get(options), Utils), () => {
			set(cleanOptions, Object.fromEntries(get(options).map((option) => [
				getOptionValue(option),
				Utils.cleanWord(getOptionLabel(option))
			])));
		});

		legacy_pre_effect(() => (get(options)), () => {
			refreshValue(get(options));
		});

		legacy_pre_effect(
			() => (
				deep_read_state(isMultiSelect()),
		...
		legacy_pre_effect(() => (deep_read_state(value())), () => {
			set(selectedElementId, value() ? getOptionElementId(value()) : '');
		});

		legacy_pre_effect(
			() => (
				deep_read_state(enableSearch()),
				get(search),
				get(options)
			),
			() => {
				set(filteredOptions, enableSearch() && strict_equals(get(search), '', false) ? searchIntoOptions(get(options), get(search)) : get(options));
			}
		);

		legacy_pre_effect(
			() => (
				deep_read_state(error()),
				deep_read_state(value())
			),
			() => {
				error(value() ? '' : error());
		...
		legacy_pre_effect(
			...
			() => {
				set(ariaActiveDescendant, strict_equals(get(valueLabel), "", false)
					? {
						"aria-activedescendant": get(selectedElementId)
					}
					: {});
			}
		);
		...
		event('click', $document, checkOutterEvent);

		var input = first_child(fragment);

		remove_input_defaults(input);
		bind_this(input, ($$value) => set(rootInput, $$value), () => get(rootInput));
...

		{
			...
			if_block(node, ($$render) => {
				if (ariaRequired()) $$render(consequent);
			});
		}
		...
		reset(span_1);
		reset(div_2);
		bind_this(div_2, ($$value) => set(comboButton, $$value), () => get(comboButton));
...

		{
			var consequent_1 = ($$anchor) => {
				...
				bind_this(input_1, ($$value) => set(searchInput, $$value), () => get(searchInput));

				template_effect(
					($0) => {
						set_attribute(input_1, 'id', ids.search);
						set_attribute(input_1, 'placeholder', searchPlaceholder());
						set_attribute(input_1, 'aria-label', `Recherche parmi les ${comboAriaLabel() ?? ''}`);
						classes = set_class(input_1, 1, '', null, classes, $0);
					},
					[
						() => ({
							'search-on-going': equals$1(get(search), '', false)
						})
					],
					derived_safe_equal
				);

				bind_value(input_1, () => get(search), ($$value) => set(search, $$value));
				event('keydown', input_1, stopPropagation(handleSearchArrowKey));
		...
		each(ul, 5, () => get(filteredOptions), index, ($$anchor, option) => {
			var fragment_1 = comment();
				...
				var consequent_2 = ($$anchor) => {
					...
					template_effect(
						($0, $1, $2, $3, $4) => {
							...
							set_attribute(input_2, 'id', $2);

							if (input_2_value !== (input_2_value = $3)) {
						...
						[
							() => getOptionElementId(get(option)),
							() => value() && value().includes(getOptionValue(get(option))),
							() => getOptionValue(get(option)),
							() => getOptionValue(get(option)),
							() => getOptionLabel(get(option))
						],
						derived_safe_equal
					);
					...
					event('keydown', li, (e) => handleOptionKeyDown(e, get(option)));
					append($$anchor, li);
				};
				...
				var alternate = ($$anchor) => {
...

					{
						var consequent_3 = ($$anchor) => {
							...
							reset(span_2);
							template_effect(() => set_text(text_3, emptyOptionSrMessage()));
							append($$anchor, span_2);
						};

						if_block(node_3, ($$render) => {
							if (strict_equals(get(option)[0], '-1')) $$render(consequent_3);
						});
					}
					...
					template_effect(
						($0, $1, $2) => {
							set_attribute(li_1, 'id', $0);
							set_attribute(li_1, 'aria-selected', $1);
							set_text(text_4, ` ${$2 ?? ''}`);
						},
						[
							() => getOptionElementId(get(option)),
							() => equals$1(getOptionValue(get(option)), value()) ? 'true' : 'false',
							() => getOptionLabel(get(option))
						],
						derived_safe_equal
					);

					event('click', li_1, () => select(get(option)));
					event('keydown', li_1, (e) => handleOptionKeyDown(e, get(option)));
					append($$anchor, li_1);
				};

				if_block(node_2, ($$render) => {
					if (isMultiSelect()) $$render(consequent_2); else $$render(alternate, false);
				});
			}
		...
		reset(ul);
		bind_this(ul, ($$value) => set(listBox, $$value), () => get(listBox));
		reset(div_4);
		reset(div_3);
		reset(div_1);
...

		{
			...
			if_block(node_4, ($$render) => {
				if (equals$1(error(), '', false)) $$render(consequent_4);
			});
		}
...

		{
			var consequent_5 = ($$anchor) => {
				...
				template_effect(() => set_attribute(link, 'href', cssPath()));
				append($$anchor, link);
			};

			if_block(node_5, ($$render) => {
				if (cssPath()) $$render(consequent_5);
			});
		}
		...
		template_effect(
			($0) => {
				...
				set_attribute(input, 'id', $$restProps.id);
				set_attribute(input, 'name', $$restProps.name);
				...
				set_attribute(label_1, 'id', ids.label);
				set_attribute(label_1, 'for', ids.control);
				set_text(text, `${label() ?? ''} `);
				...
				attributes_1 = set_attributes(div_2, attributes_1, {
					...
					role: 'combobox',
					id: ids.control,
					'aria-expanded': !get(hidden),
					'aria-controls': ids.box,
					'aria-haspopup': 'true',
					'aria-labelledby': ids.label,
					'aria-required': ariaRequired(),
					disabled: strict_equals(get(options).length, 0),
					'aria-invalid': equals$1(error(), '', false),
					...get(ariaActiveDescendant)
				});

				set_text(text_1, $0);
				div_3.hidden = get(hidden);
				set_attribute(div_4, 'id', ids.box);
			},
			[
				() => topText(get(options), value(), get(valueLabel), noValueMessage(), noOptionsMessage())
			],
			derived_safe_equal
		);

		event('click', label_1, (e) => get(comboButton).focus());
		event('click', div_2, preventDefault(toggleBox));
		event('keydown', div_2, handleComboKeyDown);
		event('click', div, markInnerEvent);
		event('focusin', div, () => set(focusIn, true));

		event('focusout', div, () => // si un focusIn n'est pas suivi
		...
		event('keydown', div, handleTextKeyDown);

		event('keydown', div, function (...$$args) {
			apply(() => debounceKeyDown, this, $$args);
		});

		event('keydown', div, handleEscape);
		append($$anchor, fragment);
	...
	SelectBoxWC[FILENAME] = 'src/sdg/components/SelectBox/SelectBoxWC.svelte';

	function SelectBoxWC($$anchor, $$props) {
		check_target(new.target);
	...
	customElements.define('qc-select-box', create_custom_element(
		SelectBoxWC,
		{
			id: { attribute: 'id' },
			...
			comboAriaLabel: { attribute: 'combo-aria-label' },
			ariaRequired: {
			...
			emptyOptionSrMessage: { attribute: 'empty-option-sr-message' },
