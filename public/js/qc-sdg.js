(function () {
	...
	var root$o = add_locations(from_html(`<div></div>`), Icon[FILENAME], [[16, 0]]);
	...
	var root$n = add_locations(from_html(`<div tabindex="0"><div class="icon-container"><div class="qc-icon"><!></div></div> <div class="content-container"><div class="content"><!> <!> <!></div></div></div>`), Notice[FILENAME], [[57, 0, [[60, 2, [[61, 4]]], [69, 2, [[70, 4]]]]]]);
	...
	function Notice($$anchor, $$props) {
		...
		user_effect(() => {
			if (role && get$1(noticeElement)) {
				const tempNodes = Array.from(get$1(noticeElement).childNodes);

				get$1(noticeElement).innerHTML = "";

				// Réinsère le contenu pour qu'il soit détecté par le lecteur d'écran.
				tempNodes.forEach((node) => get$1(noticeElement).appendChild(node));
			}
		});
		...
		reset(div_4);
		bind_this(div_4, ($$value) => set(noticeElement, $$value), () => get$1(noticeElement));
		reset(div_3);
		reset(div);
	...
	function PivHeader($$anchor, $$props) {
		...
		onMount(() => {
			set(containerClass, get$1(containerClass) + (strict_equals(fullWidth(), 'true') ? '-fluid' : ''));
...

		{
...

			{
				var consequent_3 = ($$anchor) => {
					...
					a_3.__click = (evt) => {
						evt.preventDefault();
						set(displaySearchForm, !get$1(displaySearchForm));
					...
					reset(span_1);
					reset(a_3);
					template_effect(() => set_text(text_3, get$1(displaySearchForm) ? hideSearchText() : displaySearchText()));
					append($$anchor, a_3);
				};
...

			{
				var consequent_9 = ($$anchor) => {
					...
					reset(div_10);
					bind_this(div_10, ($$value) => set(searchZone, $$value), () => get$1(searchZone));
					append($$anchor, div_10);
				};

				add_svelte_meta(
					() => if_block(node_11, ($$render) => {
						if (get$1(displaySearchForm)) $$render(consequent_9);
					}),
					'if',
					PivHeader,
					134,
		...
		reset(div);
		template_effect(() => set_class(div_1, 1, get$1(containerClass)));
		append($$anchor, div);
	...
	function PivFooter($$anchor, $$props) {
		...
		add_svelte_meta(
			() => each(a, 21, () => [['light', logoSrc()], ['dark', logoSrcDarkTheme()]], index, ($$anchor, $$item) => {
				...
				let theme = () => get$1($$array)[0];

				theme();

				let src = () => get$1($$array)[1];
	...
	var root$h = add_locations(from_html(`<button><!></button>`), IconButton[FILENAME], [[16, 0]]);
	...
	var root_1$9 = add_locations(from_html(`<div role="alert"><div><div class="qc-general-alert-elements"><!> <div class="qc-alert-content"><!> <!></div> <!></div></div></div>`), Alert[FILENAME], [[40, 4, [[43, 8, [[44, 12, [[50, 16]]]]]]]]);
	...
	function Alert($$anchor, $$props) {
		...
		function hideAlert() {
			hide("true");
			get$1(rootElement).dispatchEvent(new CustomEvent('qc.alert.hide', { bubbles: true, composed: true }));
		}
...

		{
			var consequent_1 = ($$anchor) => {
				...
				reset(div_1);
				reset(div);
				bind_this(div, ($$value) => set(rootElement, $$value), () => get$1(rootElement));
	...
	var root$g = add_locations(from_html(`<!> <link rel="stylesheet"/>`, 1), AlertWC[FILENAME], [[25, 0]]);
	...
	var root$f = add_locations(from_html(`<a href="#top"><!> <span> </span></a>`), ToTop[FILENAME], [[67, 0, [[77, 3]]]]);
	...
	function ToTop($$anchor, $$props) {
		...
		let lastVisible = setContext('visible', () => get$1(visible));
		let lastScrollY = 0;
		...
		function handleScrollUpButton() {
			...
			if (!get$1(visible) && lastVisible) {
				// removing focus on visibility loss
				toTopElement.blur();
			}

			lastVisible = get$1(visible);
		...
		template_effect(() => {
			classes = set_class(a, 1, 'qc-to-top', null, classes, { visible: get$1(visible) });
			set_attribute(a, 'tabindex', get$1(visible) ? 0 : -1);
	...
	var root$e = add_locations(from_html(`<span role="img" class="qc-ext-link-img"></span>`), ExternalLink[FILENAME], [[89, 0]]);

	function ExternalLink($$anchor, $$props) {
		check_target(new.target);
		push($$props, true);

		const externalIconAlt = prop($$props, 'externalIconAlt', 23, () => strict_equals(Utils.getPageLanguage(), 'fr')
		...
		function createVisibleNodesTreeWalker() {
			return document.createTreeWalker(get$1(imgElement).parentElement, NodeFilter.SHOW_ALL, {
				acceptNode: (node) => {
		...
		onMount(() => {
			get$1(imgElement).parentElement.querySelectorAll('a').forEach((link) => {
				...
				span.appendChild(get$1(imgElement));
		...
		bind_this(span_1, ($$value) => set(imgElement, $$value), () => get$1(imgElement));
	...
	function ExternalLinkWC($$anchor, $$props) {
		...
		add_svelte_meta(() => ExternalLink($$anchor, spread_props(() => props)), 'component', ExternalLinkWC, 15, 0, { componentTag: 'ExternalLink' });
	...
	var root$d = add_locations(from_html(`<div><!> <input/> <!></div>`), SearchInput[FILENAME], [[28, 0, [[39, 4]]]]);
	...
	var root$c = add_locations(from_html(`<div><!> <!></div>`), SearchBar[FILENAME], [[37, 0]]);
	...
	function SearchBar($$anchor, $$props) {
...

		{
			...
			add_svelte_meta(
				() => SearchInput(node, spread_props(() => get$1(inputProps), {
					get value() {
						return value();
					},
	...
	var root_1$8 = add_locations(from_html(`<div role="alert"><!></div>`), FormError[FILENAME], [[35, 0]]);
			...
			defaultInvalidText = tag(
				user_derived(() => label()
					? strict_equals(lang, 'fr')
						? `Le champ ${get$1(cleanLabel)} est obligatoire.`
						: `${get$1(cleanLabel)} field is required.`
					: strict_equals(lang, 'fr')
			...
			var consequent = ($$anchor) => {
				...
				add_svelte_meta(
					() => await_block(node_1, tick, ($$anchor) => {}, ($$anchor, _) => {
						...
						html(node_3, () => invalidText() ? invalidText() : get$1(defaultInvalidText));
						reset(span);
						append($$anchor, fragment_1);
					}),
					'await',
	...
	var root_1$7 = add_locations(from_html(`<span class="qc-required" aria-hidden="true">*</span>`), LabelText[FILENAME], [[4, 26]]);
	var root$b = add_locations(from_html(`<!><!>`, 1), LabelText[FILENAME], []);
	...
	var root_1$6 = add_locations(from_html(`<fieldset><!> <div><!></div> <!></fieldset>`), Fieldset[FILENAME], [[31, 0, [[47, 4]]]]);
	...
	function Fieldset($$anchor, $$props) {
		...
		const fieldset = wrap_snippet(Fieldset, function ($$anchor) {
			...
			reset(div);
			bind_this(div, ($$value) => set(groupSelection, $$value), () => get$1(groupSelection));
	...
	function ChoiceGroup($$anchor, $$props) {
		...
		user_effect(() => {
			(host() ? host() : get$1(fieldsetElement)).querySelectorAll('input, .qc-choicefield').forEach((input) => updateChoiceInput(input, required(), invalid(), compact(), selectionButton(), inline(), name()));
		});
	...
	var root_2$4 = add_locations(from_html(`<span class="qc-required" aria-hidden="true">*</span>`), Checkbox[FILENAME], [[56, 4]]);
	var root$9 = add_locations(from_html(`<div><!> <!> <!></div>`), Checkbox[FILENAME], [[64, 4]]);
	...
	function Checkbox($$anchor, $$props) {
		...
		onMount(() => {
			...
			labelElement(get$1(rootElement)?.querySelector('label'));
			input(get$1(rootElement)?.querySelector('input[type="checkbox"]'));
	...
	function CheckboxWC($$anchor, $$props) {
		...
		onMount(() => {
			...
			onChange(get$1(input), (_invalid) => invalid(_invalid));
		});
	...
	var root_4$1 = add_locations(from_html(`<div aria-live="polite"><!></div>`), TextField[FILENAME], [[146, 8]]);
	var root_1$5 = add_locations(from_html(`<!> <!> <!> <!> <!>`, 1), TextField[FILENAME], []);
	var root_6 = add_locations(from_html(`<div class="qc-textfield"><!></div>`), TextField[FILENAME], [[170, 4]]);
	...
	function TextField($$anchor, $$props) {
		...
		const textfield = wrap_snippet(TextField, function ($$anchor) {
...

			{
				var consequent_2 = ($$anchor) => {
					...
					html(node_5, () => get$1(charCountText));
					reset(div_1);
			...
			{
				let $0 = user_derived(() => invalidText() ? invalidText() : get$1(defaultInvalidText));
		...
		onMount(() => {
			...
			if (!input()) {
				input(get$1(rootElement).querySelector('input,textarea'));
			}
		...
		user_effect(() => {
			...
			if (invalid() && get$1(textFieldRow)) {
				get$1(textFieldRow).appendChild(formErrorElement());
			}
		});
...

		{
			...
			var alternate = ($$anchor) => {
				...
				reset(div_2);
				bind_this(div_2, ($$value) => set(rootElement, $$value), () => get$1(rootElement));
	...
	var root$6 = add_locations(from_html(`<!> <link rel="stylesheet"/>`, 1), TextFieldWC[FILENAME], [[100, 0]]);
	...
	function TextFieldWC($$anchor, $$props) {
		...
		onMount(() => {
			...
			onMountInput(get$1(input), (textFieldRowParam) => set(textFieldRow, textFieldRowParam, true), (valueParam) => set(value, valueParam, true), (invalidParam) => invalid(invalidParam));
		});
		...
		user_effect(() => {
			if (!get$1(input)) return;

			if (label()) {
				get$1(input).before(get$1(labelElement));
			}

			if (description()) {
				get$1(input).before(get$1(descriptionElement));
			}

			if (maxlength()) {
				get$1(input).after(get$1(maxlengthElement));
			}
		});

		user_effect(() => {
			if (!get$1(formErrorElement)) return;

			if (get$1(textFieldRow)) {
				get$1(textFieldRow).appendChild(get$1(formErrorElement));
			} else {
				get$1(input).after(get$1(formErrorElement));
			}
		});
...

		{
			...
			add_svelte_meta(
				() => TextField(node, {
					...
					get descriptionElement() {
						return get$1(descriptionElement);
					},
	...
	var root$5 = add_locations(from_html(`<label><input type="checkbox" role="switch"/> <span><!></span> <span class="qc-switch-slider"></span></label>`), ToggleSwitch[FILENAME], [[17, 0, [[20, 4], [28, 4], [33, 4]]]]);
	...
	function ToggleSwitchWC($$anchor, $$props) {
		...
		onMount(() => {
			...
			if (get$1(parent)) {
				...
				index = get$1(parent).items.length - 1;
			}
		});

		onDestroy(() => {
			get$1(parent).items.splice(index, 1);
		});

		user_effect(() => {
			if (get$1(parent)) {
				checked(get$1(parent).items[index].checked);
...

		{
			...
			add_svelte_meta(
				() => if_block(node, ($$render) => {
					if (!get$1(parent)) $$render(consequent);
				}),
				'if',
				ToggleSwitchWC,
				56,
					...
					children: wrap_snippet(ToggleSwitchGroupWC, ($$anchor, $$slotProps) => {
						...
						add_svelte_meta(
							() => each(node, 17, items, index, ($$anchor, item, $$index) => {
								validate_binding('bind:checked={item.checked}', [], () => get$1(item), () => 'checked', 49, 12);

								{
									let $0 = user_derived(() => get$1(item).disabled ?? disabled());
									let $1 = user_derived(() => justified() ?? get$1(item).justified);
									let $2 = user_derived(() => textAlign() ?? get$1(item).textAlign);
									...
									add_svelte_meta(
										() => ToggleSwitch($$anchor, {
											...
											get checked() {
												return get$1(item).checked;
											},

											set checked($$value) {
												(get$1(item).checked = $$value);
											}
										}),
										'component',
										ToggleSwitchGroupWC,
	...
	var root_2$3 = add_locations(from_html(`<li tabindex="0" role="option"><!></li>`), DropdownListItemsSingle[FILENAME], [[113, 12]]);
	var root_1$4 = add_locations(from_html(`<ul></ul>`), DropdownListItemsSingle[FILENAME], [[111, 4]]);
	...
	function DropdownListItemsSingle($$anchor, $$props) {
		...
		function focusOnFirstElement() {
			if (get$1(displayedItemsElements) && get$1(displayedItemsElements).length > 0) {
				get$1(displayedItemsElements)[0].focus();
			}
		}

		function focusOnLastElement() {
			if (get$1(displayedItemsElements) && get$1(displayedItemsElements).length > 0) {
				get$1(displayedItemsElements)[get$1(displayedItemsElements).length - 1].focus();
			}
		}

		function focusOnFirstMatchingElement(passedValue) {
			if (get$1(displayedItemsElements) && get$1(displayedItemsElements).length > 0) {
				const foundElement = get$1(displayedItemsElements).find((el) => strict_equals(el.dataset.itemValue.toString(), passedValue.toString()));
		...
		function handleComboKey(event, index, item) {
			if (strict_equals(event.key, "ArrowDown")) {
				...
				if (get$1(displayedItemsElements).length > 0 && index < get$1(displayedItemsElements).length - 1) {
					get$1(displayedItemsElements)[index + 1].focus();
				}
			}
			...
			if (strict_equals(event.key, "ArrowUp")) {
				...
				if (get$1(displayedItemsElements).length > 0 && index > 0) {
					get$1(displayedItemsElements)[index - 1].focus();
				} else {
					focusOnOuterElement()();
				}
			...
			var consequent_1 = ($$anchor) => {
				...
				add_svelte_meta(
					() => each(ul, 23, displayedItems, (item) => item.id, ($$anchor, item, index) => {
						...
						li.__click = (event) => handleMouseUp(event, get$1(item));
						li.__keydown = (event) => handleKeyDown(event, get$1(index), get$1(item));
...

						{
							...
							var alternate = ($$anchor) => {
								...
								html(node_3, () => get$1(item).label);
								append($$anchor, fragment_1);
							};

							add_svelte_meta(
								() => if_block(node_1, ($$render) => {
									if (!get$1(item).value && !get$1(item).label) $$render(consequent); else $$render(alternate, false);
								}),
								'if',
						...
						reset(li);
						validate_binding('bind:this={displayedItemsElements[index]}', [], () => get$1(displayedItemsElements), () => get$1(index), 114, 16);
						bind_this(li, ($$value, index) => get$1(displayedItemsElements)[index] = $$value, (index) => get$1(displayedItemsElements)?.[index], () => [get$1(index)]);

						template_effect(() => {
							set_attribute(li, 'id', get$1(item).id);

							set_class(li, 1, clsx([
								"qc-dropdown-list-single",
								get$1(item).disabled ? "qc-disabled" : "qc-dropdown-list-active",
								get$1(item).checked ? selectedElementCLass : ""
							]));

							set_attribute(li, 'data-item-value', get$1(item).value);
							set_attribute(li, 'aria-selected', !!get$1(item).checked);
						});
	...
	var root_2$2 = add_locations(from_html(`<li><label class="qc-choicefield-label" compact=""><input type="checkbox" class="qc-choicefield qc-compact"/> <span> </span></label></li>`), DropdownListItemsMultiple[FILENAME], [[153, 12, [[163, 16, [[168, 20], [180, 20]]]]]]);
	var root_1$3 = add_locations(from_html(`<ul></ul>`), DropdownListItemsMultiple[FILENAME], [[147, 4]]);
	...
	function DropdownListItemsMultiple($$anchor, $$props) {
		...
		function focusOnFirstElement() {
			if (displayedItems() && displayedItems().length > 0) {
				if (displayedItems()[0].disabled) {
					get$1(displayedItemsElements)[0].closest("li").focus();
				} else {
					get$1(displayedItemsElements)[0].focus();
				}
			}
		...
		function focusOnLastElement() {
			if (displayedItems() && displayedItems().length > 0) {
				if (displayedItems()[displayedItems().length - 1].disabled) {
					get$1(displayedItemsElements)[get$1(displayedItemsElements).length - 1].closest("li").focus();
				} else {
					get$1(displayedItemsElements)[get$1(displayedItemsElements).length - 1].focus();
				}
			}
		...
		function focusOnFirstMatchingElement(value) {
			if (get$1(displayedItemsElements) && get$1(displayedItemsElements).length > 0) {
				const foundElement = get$1(displayedItemsElements).find((element) => element.value.toLowerCase().includes(value.toLowerCase()));
			...
			if (strict_equals(event.key, "ArrowDown")) {
				...
				if (displayedItems().length > 0 && index < displayedItems().length - 1) {
					if (displayedItems()[index + 1].disabled) {
						get$1(displayedItemsElements)[index + 1].closest("li").focus();
					} else {
						get$1(displayedItemsElements)[index + 1].focus();
					}
				}
			...
			var consequent = ($$anchor) => {
				...
				add_svelte_meta(
					() => each(ul, 23, displayedItems, (item) => item.id, ($$anchor, item, index) => {
						...
						li.__keydown = (e) => handleLiKeyDown(e, get$1(index));
						li.__click = (e) => handleLiClick(e, get$1(item));
						...
						input.__keydown = (e) => handleKeyDown(e, get$1(index));
						validate_binding('bind:checked={item.checked}', [], () => get$1(item), () => 'checked', 175, 28);
						validate_binding('bind:this={displayedItemsElements[index]}', [], () => get$1(displayedItemsElements), () => get$1(index), 176, 28);
						bind_this(input, ($$value, index) => get$1(displayedItemsElements)[index] = $$value, (index) => get$1(displayedItemsElements)?.[index], () => [get$1(index)]);
						...
						template_effect(() => {
							...
							set_attribute(li, 'tabindex', get$1(item).disabled ? "0" : "-1");
							set_attribute(label, 'for', get$1(item).id + "-checkbox");
							set_attribute(input, 'id', get$1(item).id + "-checkbox");
							set_attribute(input, 'name', name);
							input.disabled = get$1(item).disabled;

							if (input_value !== (input_value = get$1(item).value)) {
								input.value = (input.__value = get$1(item).value) ?? '';
							}

							set_text(text, get$1(item).label);
						});

						bind_checked(input, () => get$1(item).checked, ($$value) => (get$1(item).checked = $$value));
						append($$anchor, li);
					}),
					'each',
	...
	var root_4 = add_locations(from_html(`<span class="qc-dropdown-list-no-options"><!></span>`), DropdownListItems[FILENAME], [[100, 16]]);
	var root$4 = add_locations(from_html(`<div class="qc-dropdown-list-items" tabindex="-1"><!> <div class="qc-dropdown-list-no-options-container" role="status"><!></div></div>`), DropdownListItems[FILENAME], [[62, 0, [[97, 4]]]]);
	...
	function DropdownListItems($$anchor, $$props) {
		...
		function focus() {
			Utils.sleep(5).then(() => {
				get$1(itemsComponent)?.focusOnFirstElement();
			}).catch(console.error);
		}

		function focusOnLastElement() {
			Utils.sleep(5).then(() => {
				get$1(itemsComponent)?.focusOnLastElement();
			}).catch(console.error);
		}

		function focusOnFirstMatchingElement(value) {
			if (get$1(itemsComponent) && value && value.length > 0) {
				Utils.sleep(5).then(() => {
					get$1(itemsComponent)?.focusOnFirstMatchingElement(value);
				}).catch(console.error);
			}
		}
		...
		template_effect(() => {
			set_attribute(div, 'id', id());
			set_style(div, `--dropdown-items-height: ${get$1(usedHeight) ?? ''};`);
		});
	...
	var root_1$2 = add_locations(from_html(`<span class="qc-dropdown-choice"><!></span>`), DropdownListButton[FILENAME], [[29, 8]]);
	var root_2$1 = add_locations(from_html(`<span class="qc-dropdown-placeholder"><!></span>`), DropdownListButton[FILENAME], [[31, 8]]);
	var root$3 = add_locations(from_html(`<button><!> <span><!></span></button>`), DropdownListButton[FILENAME], [[19, 0, [[34, 4]]]]);
	...
	var root_2 = add_locations(from_html(`<div class="qc-dropdown-list-search"><!></div>`), DropdownList[FILENAME], [[334, 20]]);
	var root_3 = add_locations(from_html(`<span> </span>`), DropdownList[FILENAME], [[376, 24]]);
	var root$2 = add_locations(from_html(`<div><div><!> <div tabindex="-1"><!> <div class="qc-dropdown-list-expanded" tabindex="-1" role="listbox"><!> <!> <div role="status" class="qc-sr-only"><!></div></div></div></div> <!></div>`), DropdownList[FILENAME], [[273, 0, [[278, 4, [[297, 8, [[325, 12, [[374, 16]]]]]]]]]]);
	...
	function DropdownList($$anchor, $$props) {
		...
		let instance = tag(state(void 0), 'instance'),
			parentRow = tag(user_derived(() => get$1(instance)?.closest(".qc-formfield-row")), 'parentRow'),
			...
			selectedOptionsText = tag(
				user_derived(() => {
					if (get$1(selectedItems).length >= 3) {
						if (strict_equals(lang, "fr")) {
							return `${get$1(selectedItems).length} options sélectionnées`;
						}

						return `${get$1(selectedItems).length} selected options`;
					}

					if (get$1(selectedItems).length > 0 && value().length > 0) {
						if (multiple()) {
							return get$1(selectedItems).map((item) => item.label).join(", ");
						}

						return get$1(selectedItems)[0].label;
					}
			...
			srItemsCountText = tag(
				user_derived(() => {
					const s = get$1(displayedItems).length > 1 ? "s" : "";

					if (get$1(displayedItems).length > 0) {
						return strict_equals(lang, "fr")
							? `${get$1(displayedItems).length} résultat${s} disponible${s}. Utilisez les flèches directionnelles haut et bas pour vous déplacer dans la liste.`
							: `${get$1(displayedItems).length} result${s} available. Use up and down arrow keys to navigate through the list.`;
					}
		...
		function focusOnSelectedOption(value) {
			if (get$1(displayedItems).length > 0) {
				if (value && value.length > 0) {
					get$1(dropdownItems)?.focusOnFirstMatchingElement(snapshot(value)?.sort()[0]);
				} else {
					get$1(dropdownItems)?.focus();
				}
			}
		}
		...
		function handleOuterEvent() {
			if (!Utils.componentIsActive(get$1(instance))) {
				set(expanded, false);
			}
		}
		...
		function handleTab(event) {
			...
			Utils.sleep(5).then(() => {
				if (strict_equals(event.key, "Tab") && !Utils.componentIsActive(get$1(instance))) {
					set(expanded, false);
				}
			}).catch(console.error);
		}
		...
		function handleButtonComboKey(event, targetComponent) {
			...
			if (strict_equals(event.key, "ArrowUp")) {
				...
				if (get$1(expanded)) {
					get$1(dropdownItems)?.focusOnLastElement();
				}
			}
		}
		...
		function handlePrintableCharacter(event) {
			if (enableSearch()) {
				...
				set(hiddenSearchText, get$1(hiddenSearchText) + event.key);

				if (get$1(hiddenSearchText).length > 0 && get$1(expanded)) {
					get$1(dropdownItems)?.focusOnFirstMatchingElement(get$1(hiddenSearchText));
				}
			}
		}
		...
		function closeDropdown(key) {
			...
			if (strict_equals(key, "Escape") && get$1(button)) {
				get$1(button).focus();
			}
		}

		user_effect(() => {
			if (get$1(searchText).length > 0) {
				let newDisplayedItems = [];

				for (let i = 0; i < items().length; i++) {
					if (get$1(itemsForSearch)[i].label.includes(Utils.cleanupSearchPrompt(get$1(searchText)))) {
		...
		user_effect(() => {
			if (strict_equals(get$1(previousValue)?.toString(), value()?.toString(), false)) {
				invalid(false);
			}
		});
		...
		user_effect(() => {
			const tempValue = get$1(selectedItems)?.map((item) => item.value);
		...
		user_effect(() => {
			if (get$1(parentRow) && errorElement() && !webComponentMode()) {
				get$1(parentRow).appendChild(snapshot(errorElement()));
			}
		});
		...
		add_svelte_meta(
			() => bind_this(
				DropdownListButton(node_1, {
					...
					get selectedOptionsText() {
						return get$1(selectedOptionsText);
					},
					...
					onkeydown: (e) => {
						handleButtonKeyDown(e, enableSearch() ? get$1(searchInput) : get$1(dropdownItems));
					}
				}),
					...
					add_svelte_meta(
						() => bind_this(
							SearchInput(node_3, {
								...
								onkeydown: (e) => {
									handleArrowDown(e, get$1(dropdownItems));
									handleArrowUp(e, get$1(button));
			...
			add_svelte_meta(
				() => bind_this(
					DropdownListItems(node_4, {
						...
						focusOnOuterElement: () => enableSearch() ? get$1(searchInput)?.focus() : get$1(button)?.focus(),
						handlePrintableCharacter,
		...
		add_svelte_meta(
			() => key(node_5, () => get$1(searchText), ($$anchor) => {
				var span = root_3();
				var text = child(span, true);

				reset(span);
				template_effect(() => set_text(text, get$1(srItemsCountText)));
				append($$anchor, span);
			}),
			'key',
			DropdownList,
		...
		reset(div_3);
		reset(div_2);
		bind_this(div_2, ($$value) => set(instance, $$value), () => get$1(instance));
		reset(div_1);
		...
		template_effect(() => {
			set_class(div, 1, clsx([!get$1(parentRow) && !webComponentMode() && "qc-select"]));
			...
			set_attribute(div_3, 'hidden', !get$1(expanded));
		});
	...
	var root$1 = add_locations(from_html(`<div hidden=""><!></div> <!> <link rel="stylesheet"/>`, 1), SelectWC[FILENAME], [[119, 0], [139, 0]]);
	...
	function SelectWC($$anchor, $$props) {
		...
		onMount(() => {
			...
			if (get$1(labelElement)) {
				label(get$1(labelElement).innerHTML);
			}

			if (get$1(selectElement)) {
				multiple(get$1(selectElement).multiple);
				disabled(get$1(selectElement).disabled);
				get$1(selectElement).addEventListener("change", handleSelectChange);
				observer.observe(get$1(selectElement), observerOptions);
			}
		...
		onDestroy(() => {
			observer?.disconnect();
			get$1(selectElement).removeEventListener("change", handleSelectChange);
		});

		user_effect(() => {
			if (!get$1(selectElement)) return;
			if (!get$1(selectElement).options) return;

			internalChange = true;

			let newOptionSelected = false;

			for (const option of get$1(selectElement).options) {
			...
			if (newOptionSelected) {
				get$1(selectElement).dispatchEvent(new Event('change'));
			}
		...
		user_effect(() => {
			if (get$1(parentRow) && get$1(errorElement)) {
				get$1(parentRow).appendChild(get$1(errorElement));
			}
		});

		function setupItemsList() {
			const options = get$1(selectElement)?.querySelectorAll("option");
		...
		{
			let $0 = user_derived(() => get$1(selectElement)?.getAttribute("aria-label"));
	...
	Note[FILENAME] = 'src/sdg/components/Note/Note.svelte';

	var root_1$1 = add_locations(from_html(`<a> </a>`), Note[FILENAME], [[44, 4]]);
	var root = add_locations(from_html(`<span class="note-index"><!> <!></span>`), Note[FILENAME], [[40, 0]]);
	...
	function Note($$anchor, $$props) {
		...
		const $noteStore = () => (
			validate_store(noteStore),
			store_get(noteStore, '$noteStore', $$stores)
		);

		const [$$stores, $$cleanup] = setup_stores();

		// Extend the class so we can let it participate in HTML forms
		let self = prop($$props, 'self', 12),
			id = prop($$props, 'id', 12, ''),
			index$1 = prop($$props, 'index', 12, 0);

		let indexElement = mutable_source();

		onMount((_) => {
			self().prepend(get$1(indexElement));
		});

		function initNoteStore(id) {
			if (strict_equals(id, '')) return;

			store_mutate(noteStore, untrack($noteStore)[id] = { index: index$1(), links: [], selected: null }, untrack($noteStore));
		}
		...
		legacy_pre_effect(() => (deep_read_state(index$1())), () => {
			store_mutate(noteStore, untrack($noteStore)[id()].index = index$1(), untrack($noteStore));
		});
		...
		add_svelte_meta(() => Icon(node, { type: 'arrow-up', color: 'blue-piv', size: 'xs' }), 'component', Note, 42, 24, { componentTag: 'Icon' });
		...
		add_svelte_meta(
			() => each(
				...
				($$anchor, letter) => {
					...
					template_effect(() => {
						set_attribute(a, 'href', `#qc-note-link-${id() ?? ''}-${get$1(letter) ?? ''}`);

						classes = set_class(a, 1, '', null, classes, {
							selected: equals(get$1(letter), $noteStore()[id()].selected)
						});

						set_text(text_1, get$1(letter));
					});
		...
		reset(span);
		bind_this(span, ($$value) => set(indexElement, $$value), () => get$1(indexElement));

		template_effect(() => {
			set_attribute(span, 'id', `qc-note-${id() ?? ''}`);
			set_text(text, ` ${index$1() ?? ''} `);
		});
	...
	customElements.define('qc-note', create_custom_element(Note, { self: {}, id: {}, index: {} }, [], [], false, (customElementConstructor) => {
		// Extend the class so we can let it participate in HTML forms
		return class extends customElementConstructor {
			constructor() {
				super();
	...
	NoteContainer[FILENAME] = 'src/sdg/components/Note/NoteContainer.svelte';

	function NoteContainer($$anchor, $$props) {
		check_target(new.target);
		push($$props, false);
		...
		onMount((_) => {
			...
			// console.log(self.querySelectorAll('qc-note'))
			self().querySelectorAll('qc-note').forEach((note) => note.setAttribute('index', index++));
		});
	...
	customElements.define('qc-note-container', create_custom_element(NoteContainer, { self: {} }, [], [], false, (customElementConstructor) => {
		// Extend the class so we can let it participate in HTML forms
		return class extends customElementConstructor {
			constructor() {
				super();
	...
	NoteLink[FILENAME] = 'src/sdg/components/Note/NoteLink.svelte';

	var root_1 = add_locations(from_html(`<sup><a><span class="qc-sr-only"> </span> <span aria-hidden="true"> </span></a></sup>`), NoteLink[FILENAME], [[43, 0, [[43, 5, [[47, 4], [48, 4]]]]]]);
	...
	function NoteLink($$anchor, $$props) {
		...
		let targetId = prop($$props, 'targetId', 12),
			linkAriaLabel = prop($$props, 'linkAriaLabel', 28, () => strict_equals(lang, 'fr')
				? "Aller à la note %note-index%"
				: "Go to note %note-index%");

		let letter = mutable_source(),
			noteIndex = mutable_source();
		...
		legacy_pre_effect(() => (get$1(letter), $noteStore(), deep_read_state(targetId())), () => {
			if (!get$1(letter) && $noteStore()[targetId()]) {
				set(letter, indexToLetter($noteStore()[targetId()].links.length + 1));
				store_mutate(noteStore, untrack($noteStore)[targetId()].links = [...$noteStore()[targetId()].links, get$1(letter)], untrack($noteStore));
			}
		});

		legacy_pre_effect(() => ($noteStore(), deep_read_state(targetId())), () => {
			set(noteIndex, $noteStore()[targetId()].index);
		});
...

		{
			var consequent = ($$anchor) => {
				...
				template_effect(
					($0) => {
						set_attribute(a, 'href', `#qc-note-${targetId() ?? ''}`);
						set_attribute(a, 'id', `qc-note-link-${targetId() ?? ''}-${get$1(letter) ?? ''}`);
						set_text(text, $0);
						set_text(text_1, `${get$1(noteIndex) ?? ''}${get$1(letter) ?? ''}`);
					},
					[
						() => (
							deep_read_state(linkAriaLabel()),
							get$1(noteIndex),
							untrack(() => linkAriaLabel().replace('%note-index%', get$1(noteIndex)))
						)
					]
				);

				event('click', a, (_) => store_mutate(noteStore, untrack($noteStore)[targetId()].selected = get$1(letter), untrack($noteStore)));
				append($$anchor, sup);
			};
	...
	customElements.define('qc-note-link', create_custom_element(
		NoteLink,
		{
