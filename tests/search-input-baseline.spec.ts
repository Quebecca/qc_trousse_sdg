...
test('SearchInput baseline — debounce regroupe les frappes en un seul événement', {
    ...
    // Écouter les événements qc-change sur le custom element
    await page.evaluate(() => {
        const events: string[] = [];
        const el = document.querySelector('qc-search-input[debounce="300"]');
...
test('SearchInput baseline — debounce réinitialise le timer à chaque frappe', {
    ...
    // Écouter les événements qc-change
    await page.evaluate(() => {
        const events: string[] = [];
        const el = document.querySelector('qc-search-input[debounce="300"]');
        el?.addEventListener('qc-change', (e: any) => {
    ...
    // Attendre que le debounce final se déclenche
    await page.waitForTimeout(200);
...
test('SearchInput baseline — clear annule le debounce en attente', {
    ...
    const clearBtn = page.locator('qc-search-input[debounce="300"]')
        .getByRole('button', { name: 'Effacer le texte' });
