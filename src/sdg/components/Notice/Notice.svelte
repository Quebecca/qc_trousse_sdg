<script>
  ...
  const usedType = $derived(types.includes(type) ? type : defaultType);
  const usedHeader = $derived(header.match(/h[1-6]/) ? header : defaultHeader);
  const role = $derived(usedType === "success" ? "status" : (usedType === "error" ? "alert" : null));
  ...
  const shouldUseIcon = $derived(usedType === "advice" || usedType === "note");
